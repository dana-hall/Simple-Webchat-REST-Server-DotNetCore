...
namespace webchat.DAO
{
    public class MessageDAO : IMessageDAO
    {
        ...
        public MessageDAO(IConfiguration config)
        {
            // Get configuration info from appsettings.json
            var url = config.GetSection("MongoDB:url").Value;
            var port = config.GetSection("MongoDB:port").Value;
            var database = config.GetSection("MongoDB:database").Value;
            var document = config.GetSection("MongoDB:document").Value;
        ...
        public UpdateResponseMessage updateMessage(String id, UpdateRequestMessage req)
        {
            ...
            try
            {
                ...
                response = new UpdateResponseMessage(Status.STATUS.SUCCESS, oid, req, result);
            }
            catch (MongoWriteException err)
            {
                response = new UpdateResponseMessage(Status.STATUS.ERROR, oid, req, err);
            }
